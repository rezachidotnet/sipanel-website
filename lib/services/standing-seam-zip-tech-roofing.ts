import standingSeamPageSpec from '@/specs/pages/standing_seam_zip_tech_roofing_page.json';
...
type StandingSeamPageSpec = {
  page: 'standing_seam_zip_tech_roofing';
  ...
  page_sections: StandingSeamSection[];
};

const page = standingSeamPageSpec as StandingSeamPageSpec;

function getSection<T extends StandingSeamSection['id']>(id: T): Extract<StandingSeamSection, {id: T}> {
  const section = page.page_sections.find((item): item is Extract<StandingSeamSection, {id: T}> => item.id === id);

  if (!section) {
    throw new Error(`Missing ${id} in standing_seam_zip_tech_roofing_page.json`);
  }

  return section;
}

export const standingSeamZipTechRoofingSpec = page;

export function getStandingSeamZipTechRoofingPage(): ServicePageTemplateData {
  ...
  return {
    ...
    technicalProof: {
      ...
      assets: technicalProof.items.map((item) => ({
        title: item.title,
        description: `${item.description} Pending real roofing asset.`,
        assetStatus: 'pending'
      }))
    },
    processWorkflow: {
      title: 'Roofing Execution Workflow',
      steps: solution.steps
    },
    qualityCheckpoints: {
    ...
    faq: {
      ...
      items: faq.items.map((item) =>
        item.question === 'Can SIPANEL help with roof repair or only new roofing?'
          ? {
              question: item.question,
              answer: 'New industrial roofing is supported by this page. Existing leakage review or repair service availability is pending internal confirmation.'
            }
          : item
      )
    },
